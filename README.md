...
到力矩级柔顺控制的完整链路。每个模块都配可复现的定量验证实验，**734 个测试全部通过**
...
[`docs/控制方法/`](docs/控制方法/) **13 篇**，覆盖从运动学到辨识的每一层，统一按
...
| 10 | [运动学与雅可比](docs/控制方法/10_运动学与雅可比.md) | TCP、雅可比、任务空间惯量、夹爪开口的统一口径 |
| 11 | [逆运动学与奇异性](docs/控制方法/11_逆运动学与奇异性.md) | SO(3) 连续误差、DLS、零空间投影 |
| 12 | [轨迹与路径规划](docs/控制方法/12_轨迹与路径规划.md) | 路径搜索 → 碰撞 → 倒角 → 时间参数化 → 在线重规划 |
| 13 | [状态估计](docs/控制方法/13_状态估计.md) | IMU + 运动学融合，位置/速度/姿态/零偏 |
